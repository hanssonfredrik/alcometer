...
npm run icons     # regenerate PWA icons from assets/*.svg (run after editing them)
```
...
src/
  ...
  lib/                  Pure, framework-free logic (unit-test friendly):
    ...
    storage.js          localStorage load/save + defaults/backfill.
...
One object, persisted to `localStorage` under `neontrk_v1`:
